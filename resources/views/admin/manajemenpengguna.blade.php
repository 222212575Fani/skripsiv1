<x-layoututama title="Manajemen Pengguna">
    <div x-data="{ 
        ...
        <div class="hidden xl:flex items-center justify-between gap-4 border-b border-gray-200/80 px-1">
            {{-- Underline Tabs --}}
            <div id="pengguna-tabs-wrapper" class="flex items-center gap-6 overflow-x-auto text-xs font-semibold scrollbar-none">
                @foreach($statuses as $key => $label)
                    <a href="{{ route('admin.manajemenpengguna', array_merge(['status' => $key], request('search') ? ['search' => request('search')] : [])) }}"
                       class="pb-3 flex items-center gap-2 transition-all relative whitespace-nowrap {{ $currentStatus == $key ? 'text-[#604EE6] border-b-2 border-[#604EE6] font-bold' : 'text-gray-500 hover:text-gray-800' }}">
                        <span>{{ $label }}</span>
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $currentStatus == $key ? 'bg-purple-100 text-[#604EE6]' : 'bg-gray-100 text-gray-500' }}">
                            {{ $counts[$key] ?? 0 }}
                        </span>
                    </a>
                @endforeach
            ...
            <x-datatable :paginator="$users" item-name="data pengguna" :card="false">
                ...
                <tr class="bg-white hover:bg-[#F8F7FF] transition-colors align-middle hidden xl:table-row border-b border-gray-100 last:border-none">
                    ...
                    <td class="py-4 px-4 text-xs align-middle">
                        @if($user->role)
                            <span class="font-light text-gray-700">{{ $user->role->nama_role }}</span>
                        @else
                                    ...
                                    <div class="flex items-start gap-3.5">
                                        ...
                                        <div class="flex-1 min-w-0 flex flex-col gap-1.5">
                                            ...
                                            <div class="flex items-center gap-1.5 text-xs pt-0.5">
                                                @if($user->role)
                                                    <div class="inline-flex items-center gap-1.5 text-gray-600 font-light">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-[#604EE6] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                ...
                                                @else
                                                    <div class="inline-flex items-center gap-1.5 text-amber-600 font-light">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
