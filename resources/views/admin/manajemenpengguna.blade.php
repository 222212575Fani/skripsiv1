    ...
    <div x-data="{}">
        ...
        <div class="flex flex-col gap-4">
            <div class="flex justify-between items-end px-2">
                ...
                {{-- Tombol Tambah Pengguna dengan Trigger Alpine.js --}}
                    ...
                    @foreach(['semua' => 'Semua', 'pending' => 'Pending', 'aktif' => 'Aktif', 'non-aktif' => 'Non-Aktif'] as $key => $label)
                        <a href="{{ route('admin.manajemenpengguna', ['status' => $key]) }}" 
                           class="pb-4 text-[11px] uppercase tracking-widest font-black transition-all border-b-2 {{ $currentStatus == $key ? 'border-[#5C46F5] text-[#5C46F5]' : 'border-transparent text-gray-400 hover:text-gray-600' }}">
                            {{ $label }}
                            @php $cKey = ($key == 'non-aktif') ? 'nonaktif' : $key; @endphp
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-gray-400 text-[10px] uppercase tracking-widest font-bold">
                                ...
                                <th class="px-6 py-6 w-[25%]">Nama</th>
                            ...
                            <tr class="group hover:bg-gray-50/50 transition-all duration-200 border-t border-gray-50">
                                ...
                                <td class="px-6 py-6">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-[#5C46F5] flex items-center justify-center text-white text-[10px] font-bold">
                                ...
                                <td class="px-6 py-6 text-center">
                                    <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase border {{ $user->status_akun == 'aktif' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-amber-50 text-amber-600 border-amber-100' }}">
                                        {{ $user->status_akun }}
                                    </span>
                                </td>
                                <td class="px-6 py-6 text-center">
