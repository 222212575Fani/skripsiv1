<x-layoututama title="Dashboard Ketua Tim">
    <div class="flex flex-col gap-8 pb-10" 
        ...
        @if($timKerja)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-3.5 xl:gap-4">
            <x-cardstatistikdashboard 
                title="Total Proyek" 
                value="{{ $statsTim['total'] ?? $totalProyek }}" 
                percent="{{ $statsTim['total_persen_text'] ?? 'Total proyek tim' }}" 
        ...
        @if($timKerja)
        <div class="bg-white rounded-2xl sm:rounded-[28px] shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8 flex flex-col gap-4 sm:gap-6">
                ...
                <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-2.5 w-full lg:w-auto">
                    ...
                    <div class="relative w-full lg:w-60">
                        <div class="flex items-center gap-3 px-4 py-2 bg-white hover:bg-[#F8F7FF] focus-within:bg-white border border-purple-200 hover:border-purple-300 focus-within:border-[#604EE6] focus-within:ring-2 focus-within:ring-purple-100 rounded-full text-xs font-normal text-[#604EE6] transition-all shadow-2xs w-full">
                            <span class="text-[#604EE6] shrink-0">
                            ...
                            <button type="button" x-show="search" @click="search = ''" class="text-[#604EE6] hover:text-[#524397] transition-colors shrink-0 cursor-pointer" title="Hapus pencarian">
                    ...
                    <div class="relative w-full lg:w-auto shrink-0">
                        <button @click="filterOpen = !filterOpen" @click.outside="filterOpen = false" type="button" 
                            ...
                            <div class="flex items-center gap-1.5 truncate">
                                ...
                                <span class="truncate max-w-35" :class="status === 'semua' ? 'text-gray-400 font-light' : 'text-gray-700 font-normal'" x-text="
            ...
            {{-- DOT PAGINATION DENGAN INDIKATOR PILL & BULAT (15 Proyek Per Halaman) --}}
            @if(isset($proyekTim) && method_exists($proyekTim, 'hasPages') && $proyekTim->hasPages())
                <div class="flex items-center justify-center gap-2 sm:gap-2.5 mt-8 mb-4">
                    @foreach ($proyekTim->getUrlRange(1, $proyekTim->lastPage()) as $page => $url)
                        @if ($page == $proyekTim->currentPage())
