<x-layoututama title="Manajemen Proyek">
    <div x-data="{}" class="flex flex-col gap-6 w-full">
        ...
        <div class="bg-white rounded-[28px] shadow-xs border border-gray-100 overflow-hidden w-full p-6 flex flex-col gap-6">
            ...
            {{-- BARIS ATAS: JUDUL, LIVE SEARCH, STATUS FILTER DROPDOWN, DAN TOMBOL TAMBAH --}}
            <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 px-2">
                ...
                {{-- KONTROL DESKTOP (Hanya Tampil di Layar >= xl): Search, Filter Status, Tombol Tambah, Reset --}}
                <div class="hidden xl:flex items-center gap-3">
                    {{-- Live Search --}}
                    <div class="flex items-center gap-3 px-4 py-2 bg-white hover:bg-[#F8F7FF] focus-within:bg-white border border-purple-200 hover:border-purple-300 focus-within:border-[#6E5BC3] rounded-full text-xs font-normal text-[#6E5BC3] transition-all shadow-2xs w-60"
                    ...
                    <div class="relative" x-data="{ statusOpen: false }" @click.outside="statusOpen = false">
                        <button @click="statusOpen = !statusOpen" type="button" 
                            ...
                            <span class="truncate text-gray-700">{{ $statuses[$currentStatus] ?? 'Semua Proyek' }}</span>
                    ...
                    @if($currentStatus !== 'semua' || request('search'))
                        <a href="{{ route('ketuatim.manajemenproyek') }}" 
            ...
            {{-- KONTROL MOBILE / TABLET (< xl): Berurutan ke Bawah 3: 1) Tambah Proyek Baru, 2) Cari Nama Proyek, 3) Filter Status --}}
