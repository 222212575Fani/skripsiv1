...
<div class="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm space-y-5"
    ...
    {{-- Header Card & Tombol Pill Filter Periode Memanjang di Bawah Judul --}}
    <div class="flex flex-col gap-3">
        <div>
            <h3 class="text-base font-bold text-gray-900 tracking-tight">{{ $title }}</h3>
        </div>

        {{-- Tombol Pill Filter Periode Memanjang Penuh di Bawah Judul --}}
        <div class="relative w-full">
            <button @click="openPeriodeDropdown = !openPeriodeDropdown" @click.outside="openPeriodeDropdown = false" type="button" 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-[#6E5BC3] transition-transform duration-200 shrink-0" :class="openPeriodeDropdown ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                </svg>
            </button>

            {{-- Dropdown Popup Memanjang Selaras Card (mt-2, w-full) --}}
            <div x-show="openPeriodeDropdown" x-cloak 
                class="absolute left-0 right-0 mt-2 w-full bg-white border border-purple-100 rounded-[24px] shadow-xl p-4 z-50 space-y-3 text-xs">
    ...
    {{-- List Ketua Proyek (1 Card 1 Baris Penuh) --}}
    <div id="cardmember-members-wrapper" 
        ...
        <div class="flex items-center justify-between p-4 rounded-2xl bg-[#F8F7FF] border border-[#EDE9FE] hover:bg-[#EDE9FE] hover:border-[#DDD6FE] hover:shadow-sm transition-all duration-200 group">
            <div class="flex items-center gap-3.5 min-w-0">
                <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-[#6E5BC3] to-[#8470E5] flex items-center justify-center text-white font-bold text-xs shadow-sm shrink-0">
