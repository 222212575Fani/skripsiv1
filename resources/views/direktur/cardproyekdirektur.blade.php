...
<div class="bg-white border border-purple-100 hover:border-purple-300 rounded-[28px] p-6 h-full flex flex-col justify-between gap-5 shadow-xs transition-all"
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-2.5 text-xs text-gray-700">
            ...
            <span class="font-medium truncate">{{ $ketuaNama }}</span>
        </div>
        ...
        <div class="max-h-64 overflow-y-auto pr-1 flex flex-col gap-3 custom-scrollbar">
            @forelse($aktivitasList as $akt)
                ...
                <div @click="$dispatch('open-modal-detail-aktivitas', {
                        ...
                        pj: @js($akt->penanggungJawab->nama ?? $akt->pj ?? '-'),
                        pm: @js($ketuaNama),
                        progress: @js($targetAkt),
                        status: @js($statusAkt),
                        tglMulai: @js($tglMulaiAkt ?? '-'),
                        tglSelesai: @js($tglSelesaiAkt ?? '-'),
                    ...
                    <div class="flex flex-col gap-1.5 pt-0.5">
                        <div class="flex items-center justify-between text-[11px]">
                            ...
                            <span class="font-normal text-gray-900">{{ $targetAkt }}%</span>
                        </div>
                        <div class="w-full bg-purple-100/60 rounded-full h-1.5 overflow-hidden">
                            <div class="bg-[#6E5BC3] h-full rounded-full transition-all duration-300" style="width: {{ $targetAkt }}%;"></div>
                        </div>
                    </div>
