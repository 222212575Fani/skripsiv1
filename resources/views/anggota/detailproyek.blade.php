<x-layoutfull title="Detail Proyek & Aktivitas">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <div class="relative flex-1 md:w-64 group/search" x-data="{ search: '{{ request('search') }}' }">
                        ...
                        <input type="text" x-model="search" 
                            ...
                            class="w-full pl-10 pr-4 py-2.5 bg-gray-50/50 hover:bg-[#F8F7FF] border border-gray-200 hover:border-[#5C46F5] rounded-2xl text-xs font-normal text-gray-700 placeholder:text-[#9E8CE3] focus:outline-none focus:border-[#5C46F5] transition-all">
                    </div>

                    @if($isKetuaProyek)
                        <button type="button" @click="$dispatch('open-modal-tambah-aktivitas')" class="px-4 py-2.5 bg-[#5C46F5] text-white rounded-2xl text-xs font-semibold hover:bg-[#4A38D4] transition-all flex items-center gap-2 shadow-sm shadow-[#5C46F5]/30 shrink-0 cursor-pointer">
    ...
    {{-- CSS KUSTOM DATATABLE & DATE PICKER --}}
    <style>
        .pj-dropdown-scroll {
        ...
        .pj-dropdown-scroll::-webkit-scrollbar {
            width: 8px;
        }
        .pj-dropdown-scroll::-webkit-scrollbar-track {
            background: #F8F7FF;
            border-radius: 9999px;
        }
        .pj-dropdown-scroll::-webkit-scrollbar-thumb {
            background: #9E8CE3;
            border: 2px solid #F8F7FF;
            border-radius: 9999px;
        }
        .pj-dropdown-scroll::-webkit-scrollbar-thumb:hover {
            background: #6E5BC3;
        }
